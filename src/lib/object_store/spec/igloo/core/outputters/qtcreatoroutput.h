...
#ifndef IGLOO_QTCREATOROUTPUT_H
#define IGLOO_QTCREATOROUTPUT_H

namespace igloo {

  class QTCreatorResultsOutput : public TestResultsOutput
  {
	public:
	  QTCreatorResultsOutput(std::ostream& outstream = std::cout) : TestResultsOutput(outstream) {}
	  ...
	  void PrintResult(const TestResults& results) const
	  {
		for (TestResults::FailedTestsType::const_iterator it = results.FailedTests().begin(); it != results.FailedTests().end(); it++)
		{
		  ...
		  {
			std::cerr <<  result.Filename() << ":" << result.LineNumber() << ": error: " << result.GetErrorMessage();
		  }
		  else
		  {
